...
AWS_BUCKET_NAME = "hx-hxip-metro"
AWS_KEY_NAME = "tiles/OSM/WGS84/"
...
def upload_file(path, filename, bucket_name, key_name):
    bucket = get_bucket(bucket_name)
    full_path = os.path.join(path, filename).replace('\\', '/')
    key_path = key_name + full_path[len(path):]
    with open(full_path, 'rb') as data:
        bucket.put_object(Key=key_path, Body=data)

def upload_folder(path, bucket_name, key_name):
    bucket = get_bucket(bucket_name)
...
class RenderThread:
    ...
    def render_tile(self, tile_uri, x, y, z):
        ...
        upload_file(self.tile_dir, filename[len(self.tile_dir):], AWS_BUCKET_NAME, AWS_KEY_NAME)
